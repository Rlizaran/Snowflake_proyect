...
SHOW TABLES IN DATABASE DEV_CITYBIKE_BRONZE;
SHOW STAGES IN DATABASE DEV_CITYBIKE_BRONZE;
SHOW FILE FORMATS IN DATABASE DEV_CITYBIKE_BRONZE;
SHOW PROCEDURES IN SCHEMA DEV_CITYBIKE_BRONZE.CITYBIKE;
SHOW PROCEDURES IN SCHEMA DEV_CITYBIKE_BRONZE.NOAA;
...
silver_counts AS (
    SELECT  
        COUNT(*) AS n_silver
    FROM DEV_CITYBIKE_SILVER.DBT_RLIZARAN_CITYBIKE.STG_CITYBIKE__CITYBIKE_TRIPS_JC
)
SELECT 
    b.n_bronze,
    s.n_silver,
...
first_error_message, 
last_load_time
FROM TABLE(DEV_CITYBIKE_BRONZE.INFORMATION_SCHEMA.COPY_HISTORY(
...
SELECT * 
FROM DEV_CITYBIKE_SILVER.DBT_RLIZARAN_NOAA.SLV_WEATHER_DAILY
where snow_depth_mm > 0.01;

select * from DEV_CITYBIKE_SILVER.DBT_RLIZARAN_NOAA.SLV_WEATHER_OBSERVATION
where element_code = 'SNOW';

select * from DEV_CITYBIKE_SILVER.DBT_RLIZARAN_INTERMEDIATE.STG_NOAA__NOAA_RAW_YEAR
where element = 'SNOW';
